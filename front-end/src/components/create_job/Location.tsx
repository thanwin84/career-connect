...
import { ComboBoxListItem } from "../ui/combobox";
import { City, CountryList } from "../../types";
...
function filterList(list:CountryList, searchTerm:string){
    const filteredList = list?.filter(item => item.name.toLowerCase().startsWith(searchTerm.toLowerCase()))
    return filteredList 
}
...
export default function Location({
    ...
    function handleCountrySelect(value:string){
        const country = countryList.find(item => item.name.toLowerCase() === value.toLowerCase())
