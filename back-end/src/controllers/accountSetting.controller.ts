...
const toggleTwoStepAuthentication = asyncHandler(async (req:Request, res:Response)=>{
    const user = await User.findById(req.user.userId)
    if (!user){
        throw new NotFoundError(`user with id ${req.user.userId} not found`)
    }
    if (!user.phoneNumber){
        throw new BadRequestError("Please provide your phone number to enable this")
    }
